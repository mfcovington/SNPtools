...
for (i in 1:length(chromosomes)) { #length(chromosomes)
    ...
    ILs_to_input <- as.character(IL_filenames[(IL_filenames$IL_chr == chromosomes[i]) & (IL_filenames$IL_chr == IL_filenames$data_chr), ]$filename)
    ...
    ggsave(paste("example.", chr_formatted, ".png", sep = ""), plot = geno, width = 6, height = 5) #CHANGED
}
